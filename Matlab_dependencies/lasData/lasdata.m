classdef lasdata < handle
    ...
    %
    %   Copyright (C) Teemu Kumpumäki / Tampere University of Technology 2014
...
methods (Access=public)
        ...
        while k<=length(varargin)
            switch( varargin{k} )
                ...
                case 'classification'
                    ...
                    k = k+2;
                case 'return_number'  %%%% -----  modified code carefull ------%%%
                    val = varargin{k+1};                    
                    if ~isnumeric(val)
                        error('return_number comparison requires a vector input of return_number codes.')
                    end                        
                    %reload if data is not in the original condition
                    if isempty(obj.get_return_number) || ( length(obj.get_return_number) ~= length(obj.selection) )
                        obj.get_return_number;
                    end

                    obj.selection = obj.selection & ( ismember(obj.get_return_number, val) );
                    k = k+2;              %%%         ----------          %%%%
                case 'user_data'
                    val = varargin{k+1};                    
    ...
    function obj = read_classification(obj,donotfilter)
        ...
        offsettable = [15 15 15 15 15 15 16 16 16]; 
    ...
    function obj = read_scan_angle(obj,donotfilter)
        ...
        offsettable = [16 16 16 16 16 16 17 17 17 17 17];
    ...
    function obj = read_user_data(obj,donotfilter)
        ...
        OFFSET = 17+obj.header.offset_to_point_data;  
        DATATYPE ='*uint8';
        DATASIZE = 1;
...
methods (Access=private)    
    ...
    function obj = las_read( obj, file )
        ...
        if obj.header.version_major == 1 && obj.header.version_minor == 0
            check = fread(fid,4,'uint32');  
            if check ~= hex2dec('CCDD')
                fclose(fid);
                error('File position do not match point data record.')
            end
        end
    ...
    function obj = readheader(obj,fid)
        ...
        try 
            ...
            if obj.header.version_minor > 3 %1.4       
                ...
                if obj.header.number_of_point_records == 0
                    obj.header.number_of_point_records = fread(fid,1,'uint64');
                else
                    %dump results
                    warning('Legacy number of point records found being nonzero. Using larger nonzero')
                    tmp = fread(fid,1,'uint64'); %legacy field contains values, but it shouldn't
                    if obj.header.number_of_point_records < tmp
                        obj.header.number_of_point_records = tmp;
                    end
                end
                if all(obj.header.number_of_points_by_return == 0)
                    obj.header.number_of_points_by_return = fread(fid,15,'uint64');
                else
                    %dump results
                    warning('Legacy number of points by return found being nonzero. Using larger nonzero')
                    tmp = fread(fid,15,'uint64'); %legacy field contains values, but it shouldn't
                    if sum(obj.header.number_of_points_by_return) < sum(tmp)
                        obj.header.number_of_points_by_return = tmp;                      
                    end
                end
            end        
        catch ex
    ...
    function obj = writeheader(obj,fid)
        ...
        if length(obj.header.number_of_points_by_return) < 6
            fwrite(fid, obj.header.number_of_points_by_return,'uint32');
        else
    ...
    function obj = read_variable_records(obj,fid)
        for k=1:obj.header.number_of_variable_records
            ...
            obj.variablerecords(k).data = uint8(fread(fid,obj.variablerecords(k).record_length,'uint8'));
    ...
    function obj = read_extended_variables(obj,fid)
        for k=1:obj.header.number_of_extended_variable_length_record
            obj.extendedvariables(k).reserved = fread(fid,2,'uint16');
            obj.extendedvariables(k).user_id = fread(fid,16,'int8');
            obj.extendedvariables(k).record_id = fread(fid,16,'uint16');
            ...
            if isfield(obj.header,'start_of_waveform_data') && ftell(fid) == obj.header.start_of_waveform_data
                ...
                obj.extendedvariables(k).data = fread(fid,obj.extendedvariables.record_length_after_header,'uint8');
            end
        end
    end
